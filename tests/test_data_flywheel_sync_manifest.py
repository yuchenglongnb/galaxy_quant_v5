...
def test_blocked_provider_with_sector_evidence_is_sector_only(tmp_path):
    ...
    assert row["candidate_data_provider"] == "amazingdata"
    assert row["candidate_provider_status"] == "blocked"
    assert row["candidate_provider_error_type"] == "login_control_flow_or_permission_blocked"
    assert row["evidence_provider"] == "ifind_mcp"
    assert row["evidence_provider_status"] == "sector_only_available"
